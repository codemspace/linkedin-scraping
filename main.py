...
email = 'Nicedeveloper1222@proton.me'
password = 'aifrysakrau1234'
...
if __name__ == '__main__':
    ...
    parser.add_argument('-in', '--index_number',
                        help="Pass --index_number to the mail and the name of octo profile", type=int, default=1, required=True)
    ...
    input_email = WebDriverWait(driver, 20).until(EC.visibility_of_element_located(
        (By.XPATH, '//*[@id="username"]'))).send_keys(email)
    input_password = WebDriverWait(driver, 20).until(EC.visibility_of_element_located(
        (By.XPATH, '//*[@id="password"]'))).send_keys(password)
    signin = WebDriverWait(driver, 20).until(EC.visibility_of_element_located(
    ...
    for i in range(0, 100):
        ...
        try:
            # Replace with the text of the button you want to find
            button_text = "Load more comments"
            xpath_expression = f"//span[text()='{button_text}']"
            WebDriverWait(driver, 20).until(
                EC.visibility_of_element_located((By.XPATH, xpath_expression))).click()
            print("pressed")
        except:
